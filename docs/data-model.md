...
> 结论先行：Career OS 的核心是 `data/career_jobs.sqlite`（schema v23）。
...
- **迁移规则**：只新增表/列，不删除或改写既有求职数据；当前 `career_os_schema_migrations` 已到 v23
...
| 表 | 行数(约) | 作用 |
|---|---:|---|
| `companies` | 113 | 企业档案：行业、类型、城市、官网、校招 URL、校招限投数 `max_campus_applications`、投递规则 `campus_application_rules` (v13)、分层 `pool`=`applied`/`sme_startup`/`archive_big` (v23) |
...
| `job_apply_runs` | 0 | 单岗投递编排器（v21–v23）：`apply_mode=precision|volume`。专投会审后先 `awaiting_human`（只出填写载荷）；海投选分轨简历后 `volume_ready`。`released` 仅在人工确认网页已提交或旧 `release` 路径。LLM 对话不当真相；禁止自动点网页提交 |
