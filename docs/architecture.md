...
│  data/career_jobs.sqlite: 核心库（权威事实源）       │
│  data/tracker.tsv: 投递追踪（派生视图，目标态）      │
│  角色: 个性化上下文，Skill 运行的参数来源            │
└──────────────────────────────────────────────────┘
```

> **权威原则（2026-09-05）**：`data/career_jobs.sqlite` 是唯一权威事实源，md/TSV 文档只是库的规范描述或派生视图；冲突时以库为准。库表结构与文档对应关系见 `docs/data-model.md`。
