...
def print_report(res: Dict, jd_title: str, resume_path: str):
    ...
    elsewhere = s['keywords_grounding'].get('elsewhere') or []
    if elsewhere:
        print(f"⚠️ 仅在非项目位置出现 ({len(elsewhere)}个，不计落地):")
        print(f"   👉 {', '.join(elsewhere)}")
        print()

    if s['keywords_grounding']['missing']:
        print(f"❌ 项目正文缺失词 Missing Keywords ({len(s['keywords_grounding']['missing'])}个):")
