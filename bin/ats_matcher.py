...
def main():
    ...
    parser.add_argument("--job-context", help="扩展捕获的 JobContext JSON（优先于 --jd-text/--jd-file）")
    ...
    if args.job_context:
        ...
        ctx = load_context_file(args.job_context)
        jd_info = to_jd_info(ctx)
        page_filters = ctx.get("hard_filters")
    elif args.job_id:
        ...
        print("错误: 必须指定 --job-context、--job-id、--jd-file 或 --jd-text 之一")
        sys.exit(2)

    scorer = ATSEngine(jd_info, resume_text)
    results = scorer.run_full_diagnosis()
    if page_filters:
        results["page_hard_filters"] = page_filters
    ...
    if args.json:
        ...
        if page_filters:
            print("📋 页面硬门槛（仅展示，尚未作为熔断；来自 JobContext）:")
            print("   " + json.dumps(page_filters, ensure_ascii=False))
            print()
